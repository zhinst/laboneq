...
def sweep_range(
    ...
    **kwargs: dict[str, object],
    ...
    """Returns a sweep context manager for the specified range.
    ...
    Arguments:
        ...
        count:
            The number of values in the sweep.
        uid:
            An optional uid passed to `sweep`. Defaults to `axis_name` if not
            provided.
        axis_name:
            An optional axis name passed to `LinearSweepParameter`.
        **kwargs:
            Additional arguments passed to `sweep`.

    Upon entering the returned context manager a parameter is returned.

    Within the context block, operations and subsections are automatically added to the
    sweep section and the active section may be retrieved by calling `active_section(...)`.

    Returns:
        A sweep context manager.
    """
...
def for_each(
    ...
    **kwargs: dict[str, object],
    ...
    """Returns a sweep context manager for the specified iterable.

    The parameter is swept through the values from `np.array(iterable)`.

    Arguments:
        iterable:
            An iterable providing the values to sweep over.
        uid:
            An optional uid passed to `sweep`. Defaults to `axis_name` if not
            provided.
        axis_name:
            An optional axis name passed to `SweepParameter`.
        **kwargs:
            Additional arguments passed to `sweep`.

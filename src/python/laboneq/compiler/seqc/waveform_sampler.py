...
def sample_and_compress(
    ...
    sampled_signature = sample_waveform(
        signals,
        waveform,
        sampling_rate,
        signal_type,
    ...
    compressed_events = compress_waveform(
        sampled_signature, convert_device_type(device_type)
    )
...
def sample_integration_weight(
    pulse_id: PulseDef,
    pulse_parameters: codegen_rs.PulseParameters | None,
...
def sample_waveform(
    ...
    for pulse_part in pulses:
        pulse_def = pulse_part.pulse
        assert pulse_def.amplitude is not None, "Pulse amplitude should have been set"
        ...
        if sampled_pulse.samples_q is not None and len(sampled_pulse.samples_i) != len(
            sampled_pulse.samples_q
        ):
            _logger.warning("Expected samples_q and samples_i to be of equal length")
        ...
        if not len_i == pulse_part.length and pulse_def.samples is None:
            ...
            raise RuntimeError(
                f" Expected {pulse_part.length} samples but got {len_i};"
                f" length = {pulse_def.length},"
                f" num samples={num_samples},"
                f" length in samples={round(pulse_def.length * sampling_rate)}"
            )
        ...
        if rf_signal and pulse_part.channel is not None:
            if pulse_part.channel == 0:
                # RF signal, channel 0
                stencil_samples(pulse_part.start, sampled_pulse.samples_i, samples_i)
            elif pulse_part.channel == 1:
                # RF signal, channel 1
                stencil_samples(pulse_part.start, sampled_pulse.samples_i, samples_q)
                has_q = True
        else:
            stencil_samples(pulse_part.start, sampled_pulse.samples_i, samples_i)
            if sampled_pulse.samples_q is not None:
                stencil_samples(pulse_part.start, sampled_pulse.samples_q, samples_q)
                has_q = True
        if pulse_part.channel is not None and device_type == DeviceType.HDAWG:
            # Markers: RF case
            if sampled_pulse.samples_marker1 is not None and pulse_part.channel == 0:
                stencil_samples(
                    pulse_part.start,
            ...
            if sampled_pulse.samples_marker1 is not None and pulse_part.channel == 1:
                stencil_samples(
                    pulse_part.start,
                    sampled_pulse.samples_marker1,
                    samples_marker2,
            ...
            if sampled_pulse.samples_marker2 is not None and pulse_part.channel == 1:
...
def _compress_qa_waveform(sampled_signature: SampledWaveformSignature) -> None:
...
def compress_waveform(
    ...
    if not _pulses_compatible_for_compression(pulse_compr_infos):
        raise LabOneQException(
    ...
    return _emit_new_awg_events(sampled_signature, new_events)

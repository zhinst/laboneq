...
def create_t1(settings: T1ExperimentSettings):
    """Create a T1 experiment parallelized to multiple qubits.
    ...
    * ac_{qubit_id}
    ...
    """
    builder = T1ExperimentBuilder()
    return builder.build(settings)


@attrs.frozen(kw_only=True)
class T1ExperimentSettings:
    """Settings for a T1 experiment parallelized to multiple qubits.
    ...
    qubit_ids: list[str]
    n_average: int
    delays: list[float] | NDArray[np.float64] = attrs.field(
        validator=attrs.validators.min_len(0)
    )
...
@attrs.define
class T1ExperimentBuilder:
    """A class for constructing single-qubit T1 experiments parallelized to
    multiple qubit control lines."""

    __experiment_name__: ClassVar[str] = "T1Experiment"
    __required_signal_types__: ClassVar[frozenset[str]] = frozenset(
        {"drive", "measure", "acquire"}
    )

    def build(self, settings: T1ExperimentSettings) -> Experiment:
        signals = [
            f"{q}_{signal_type}"

...
def create_ramsey_experiment(settings: RamseyExperimentSettings):
    """Create a Ramsey experiment parallelized to multiple qubits.
    ...
    * ac_{qubit_id}
    ...
    """
    builder = RamseyExperimentBuilder()
    return builder.build(settings)


@attrs.frozen(kw_only=True)
class RamseyExperimentSettings:
    """Settings for a Ramsey experiment parallelized to multiple qubits.
    ...
    Attributes:
        ...
        delays: Values to iterate over as the delay after π-pulse.
        excitation_pulse: Pulse to be used as π-pulse.
        ...
        excitation_frequencies: Mapping from qubit id to frequency of drive
            signals. These are the absolute frequencies of the signal that goes
            out the wire.
    ...
    qubit_ids: list[str]
    n_average: int
    delays: list[float] = attrs.field(validator=attrs.validators.min_len(0))
...
@attrs.define
class RamseyExperimentBuilder:
    """A class for constructing single-qubit Ramsey experiments parallelized to
    multiple qubit control lines."""

    __experiment_name__: ClassVar[str] = "RamseyExperiment"
    __required_signal_types__: ClassVar[frozenset[str]] = frozenset(
        {"drive", "measure", "acquire"}
    )

    def build(self, settings: RamseyExperimentSettings) -> Experiment:
        signals = [
            f"{q}_{signal_type}"
        ...
        delay_parameter = SweepParameter(values=settings.delays)
            ...
            with exp.sweep(
                ...
                for qubit_id in settings.qubit_ids:
                    ...
                    with exp.section(
                        ...
                        exp.reserve(signal=f"{qubit_id}_measure")

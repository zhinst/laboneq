...
"""Single-shot classification experiment."""
...
def create_single_shot(settings: SingleShotExperimentSettings):
    """Create a single-shot classification experiment consisting of a readout
    followed by an excitation pulse and a subsequent readout, parallelized to
    multiple qubits.
    ...
    * ac_{qubit_id}
    ...
    """
    builder = SingleShotExperimentBuilder()
    return builder.build(settings)


@attrs.frozen(kw_only=True)
class SingleShotExperimentSettings:
    """Settings for a single-shot classification experiment consisting of a
    readout followed by an excitation pulse and a subsequent readout.

    Attributes:
        qubit_ids: Labels for the qubits, e.g. ["q0", "q1", ...].
        n_average: Number of repetitions of the full experiment.
        excitation_pulse: Pulse used in drive signals.
        readout_functional: Functional to be used for measure pulses and
            acquisition kernels.
...
@attrs.define
class SingleShotExperimentBuilder:
    __experiment_name__: ClassVar[str] = "SingleShotClassification"
    __required_signal_types__: ClassVar[frozenset[str]] = frozenset(
        {"drive", "measure", "acquire"}
    )

    def build(self, settings: SingleShotExperimentSettings) -> Experiment:
        signals = [
            f"{q}_{signal_type}"
        ...
        with exp.acquire_loop_rt(
            ...
            for qubit_id in settings.qubit_ids:
                with exp.section(uid=f"{qubit_id}_readout_g"):
                    exp.play(
                ...
                with exp.section(
                    uid=f"{qubit_id}_excitation", play_after=f"{qubit_id}_readout_g"
                ):
                    exp.play(
                ...
                with exp.section(
                    uid=f"{qubit_id}_readout_e", play_after=f"{qubit_id}_excitation"
                ):
                    exp.play(
                        signal=f"{qubit_id}_measure",
                        pulse=settings.readout_functional,
                ...
                with exp.section(
                    uid=f"{qubit_id}_relaxation", length=settings.relaxation_length
                ):

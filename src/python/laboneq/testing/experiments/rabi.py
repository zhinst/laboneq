...
def create_rabi_experiment(settings: RabiExperimentSettings):
    """Create a Rabi experiment parallelized to multiple qubits.
    ...
    * ac_{qubit_id}
    ...
    """
    builder = RabiExperimentBuilder()
    return builder.build(settings)


@attrs.frozen(kw_only=True)
class RabiExperimentSettings:
    """Settings for a Rabi experiment parallelized to multiple qubits.
    ...
    Attributes:
        ...
        amplitudes: Values to iterate over as the amplitude of the π-pulse.
    ...
    qubit_ids: list[str]
    n_average: int
    amplitudes: list[float] | NDArray[np.float64] = attrs.field(
...
@attrs.define
class RabiExperimentBuilder:
    """A class for constructing single-qubit Rabi experiments parallelized to
    multiple qubit control lines."""

    __experiment_name__: ClassVar[str] = "RabiExperiment"
    __required_signal_types__: ClassVar[frozenset[str]] = frozenset(
        {"drive", "measure", "acquire"}
    )

    def build(self, settings: RabiExperimentSettings) -> Experiment:
        signals = [
            f"{q}_{signal_type}"
        ...
        amplitude_parameter = SweepParameter(values=settings.amplitudes)

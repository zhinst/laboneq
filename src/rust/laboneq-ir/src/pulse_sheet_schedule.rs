...
pub struct PulseSheetSchedule {
    ...
    /// Sampling rates in Hz per device type. Grouped by sampling rate.
    pub sampling_rates: Vec<(Vec<String>, u64)>,
}
...
impl PulseSheetSchedule {
    ...
    pub fn generate(
        root: &IrNode,
        sampling_rates: Vec<(DeviceKind, f64)>,
        expand_loops: bool,
        max_events: usize,
        id_store: &NamedIdStore,
    ) -> Self {
    ...
    fn generate_sampling_rates(sampling_rates: Vec<(DeviceKind, f64)>) -> Vec<(Vec<String>, u64)> {
        // Group devices by sampling rate
        let mut rate_groups: HashMap<u64, Vec<DeviceKind>> = HashMap::new();
        for (device_kind, rate) in sampling_rates {
            let rate_key = rate.abs() as u64;
            if !rate_groups
                .entry(rate_key)
                .or_default()
                .contains(&device_kind)
            {
                rate_groups.entry(rate_key).or_default().push(device_kind);
            }
        }

        // Ground by sampling rate and sort device names within each group
        let mut sampling_rates_vec: Vec<(Vec<String>, u64)> = rate_groups
            .into_iter()
            .map(|(rate_bits, devices)| {
                let mut sorted_devices: Vec<String> = devices
                    .into_iter()
                    .map(|d| d.to_string().to_lowercase())
                    .collect();
                sorted_devices.sort();
                (sorted_devices, rate_bits)
            })
            .collect();

        // Sort for consistent ordering
        sampling_rates_vec.sort();
        sampling_rates_vec
    }
...
mod tests {
    ...
    fn test_sampling_rates() {
        let sampling_rates = [
            (DeviceKind::Hdawg, 2.4e9),
            (DeviceKind::Shfqa, 2e9),
            (DeviceKind::Shfsg, 2e9),
            (DeviceKind::Uhfqa, 1.8e9),
        ];
        let generated_rates = PulseSheetSchedule::generate_sampling_rates(sampling_rates.to_vec());

        // Verify grouping by rate
        assert_eq!(generated_rates.len(), 3);
        assert_eq!(
            generated_rates[0],
            (vec!["hdawg".to_string()], 2_400_000_000)
        );
        assert_eq!(
            generated_rates[1],
            (
                vec!["shfqa".to_string(), "shfsg".to_string()],
                2_000_000_000
            )
        );
        assert_eq!(
            generated_rates[2],
            (vec!["uhfqa".to_string()], 1_800_000_000)
        );
    }
}

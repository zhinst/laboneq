...
/// Register experiment signals used in the experiment. This needs to be done before building the
/// experiment, so that signals can be referenced when building the experiment nodes.
///
/// The signals registered in sorted order to ensure deterministic UID assignment and
/// ordering of the said UIDs.
pub(super) fn register_experiment_signals<'py>(
    experiment: &Bound<'py, PyAny>,
    builder: &mut ExperimentBuilder<'py>,
) -> Result<()> {
    ...
    let mut signal_uid_strings = signal_uid_objects
        .iter()
        .map(|obj| obj.extract::<&str>())
        .collect::<PyResult<Vec<_>>>()?;
    signal_uid_strings.sort();

    for signal in signal_uid_strings {
        let signal_uid = builder.register_uid(signal);
        builder.register_experiment_signal(signal_uid.into());
    }
    Ok(())
}

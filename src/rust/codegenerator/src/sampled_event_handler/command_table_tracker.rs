    ...
    fn add_amplitude_config(&self, signature: &PlayWaveEvent, json: &mut Map<String, Value>) {
        if let Some(amplitude) = &signature.amplitude {
            ...
            if matches!(self.device_type, DeviceKind::HDAWG) {
                ...
                json.insert("amplitude0".into(), Value::Object(dd.clone()));
                json.insert("amplitude1".into(), Value::Object(dd));
            } else if matches!(self.device_type, DeviceKind::SHFSG) {
                json.insert("amplitude00".into(), Value::Object(dd.clone()));
                json.insert("amplitude10".into(), Value::Object(dd.clone()));
                json.insert("amplitude11".into(), Value::Object(dd));
                let mut dd = Map::new();
                dd.insert("value".into(), json!(-*value));
                if increment {
                    dd.insert("increment".into(), json!(true));
                }
                json.insert("amplitude01".into(), Value::Object(dd));
            } else {
